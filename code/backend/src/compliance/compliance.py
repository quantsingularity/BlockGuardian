...
class EnhancedComplianceManager:
    ...
    def _calculate_kyc_risk_score(
        ...
        for step in verification_result["verification_steps"]:
            if step["status"] == "failed":
                score += 15
        if not verification_result["verification_steps"][3]["success"]:
            score += 100
        if not verification_result["verification_steps"][4]["success"]:
            score += 50
    ...
    def _analyze_transaction_patterns(
        self, transaction: Transaction, session: Any
    ) -> Dict[str, Any]:
    ...
    def _generate_sar(
        self, transaction: Transaction, monitoring_result: Dict[str, Any], session: Any
    ) -> Any:
