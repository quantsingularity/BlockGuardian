...
from src.models.base import Base, AuditMixin, TimestampMixin, db_manager
from src.models.user import User, KYCStatus, AMLRiskLevel
from src.models.portfolio import Transaction, TransactionType, TransactionStatus
...
class ComplianceReport(Base, AuditMixin, TimestampMixin):
    ...
    def generate(self):
        ...
        try:
            ...
            # Add audit entry
            self.add_audit_entry("report_generated", {
            ...
            # Add audit entry
            self.add_audit_entry("report_generation_failed", {
                "report_id": self.report_id,
                "error": str(e)
            })
    ...
    def _generate_suspicious_activity_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            suspicious_activities = session.query(FraudDetection).filter(
            ...
            return {
                ...
                'records': records,
                'summary': {
                    'high_risk_count': len([r for r in records if r['risk_level'] in ['high', 'very_high', 'critical']]),
                    'blocked_transactions': len([r for r in records if r['action_taken'] == 'block']),
                    'flagged_transactions': len([r for r in records if r['action_taken'] == 'flag'])
                }
            }
        finally:
            session.close()
        ...
        try:
            ...
            for transaction in large_transactions:
                record = {
                    ...
                    'settlement_date': transaction.settled_at.isoformat() if transaction.settled_at else None,
                    'external_account': '***MASKED***' if transaction.external_account_number else None
                }
                records.append(record)
            ...
            return {
                ...
                'summary': {
                    ...
                    'average_amount': sum(float(r['amount']) for r in records) / len(records) if records else 0,
                    'transaction_types': self._count_by_field(records, 'transaction_type')
                }
            }
        finally:
            session.close()
    ...
    def _generate_kyc_status_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            users = session.query(User).filter(
                User.created_at >= self.start_date,
                User.created_at <= self.end_date
            ).all()
            ...
            for user in users:
                record = {
                    ...
                    'kyc_approved_at': user.kyc_approved_at.isoformat() if user.kyc_approved_at else None,
                    'kyc_expires_at': user.kyc_expires_at.isoformat() if user.kyc_expires_at else None,
            ...
            return {
                ...
                'summary': {
                    ...
                    'aml_risk_breakdown': self._count_by_field(records, 'aml_risk_level'),
                    'pending_kyc': len([r for r in records if r['kyc_status'] in ['not_started', 'in_progress', 'pending_review']]),
                    'expired_kyc': len([r for r in records if r['kyc_expires_at'] and r['kyc_expires_at'] < datetime.utcnow().isoformat()])
                }
            }
        finally:
            session.close()
        ...
        try:
            ...
            return {
                ...
                'summary': {
                    ...
                    'manual_review_required': len([r for r in records if r['requires_manual_review']]),
                    'high_risk_assessments': len([r for r in records if r['risk_level'] in ['high', 'very_high', 'critical']]),
                    'average_risk_score': sum(float(r['overall_risk_score']) for r in records) / len(records) if records else 0
                }
            }
        finally:
            session.close()
        ...
        try:
            ...
            return {
                ...
                'summary': {
                    ...
                    'failed_transactions': len([r for r in records if r['status'] == 'failed']),
                    'pending_transactions': len([r for r in records if r['status'] == 'pending'])
                }
            }
        finally:
            session.close()
...
class ComplianceViolation(Base, AuditMixin, TimestampMixin):
    ...
    def __repr__(self):
        return f'<ComplianceViolation {self.violation_id} - {self.violation_type.value}>'
    ...
    def assign_to_investigator(self, investigator_id: int):
        ...
        # Add audit entry
        self.add_audit_entry("violation_assigned", {
    ...
    def resolve(self, resolution_notes: str, resolved_by: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("violation_resolved", {
            "violation_id": self.violation_id,
    ...
    def mark_false_positive(self, reason: str, marked_by: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("violation_false_positive", {
            "violation_id": self.violation_id,
            "marked_by": marked_by,
            "reason": reason
        })
    ...
    def report_to_regulator(self, reported_by: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("violation_reported_to_regulator", {
            "violation_id": self.violation_id,
...
class ComplianceManager:
    ...
    def check_transaction_compliance(self, transaction: Transaction) -> List[ComplianceViolation]:
    ...
    def _create_large_transaction_violation(self, transaction: Transaction) -> ComplianceViolation:
    ...
    def _check_structuring(self, transaction: Transaction) -> Optional[ComplianceViolation]:
    ...
    def _check_transaction_velocity(self, transaction: Transaction) -> Optional[ComplianceViolation]:
        ...
        try:
            ...
            if recent_count > self.monitoring_thresholds['suspicious_velocity']:
                violation_id = f"VEL_{transaction.user_id}_{int(datetime.utcnow().timestamp())}"
    ...
    def generate_compliance_report(self, report_type: ReportType, start_date: datetime, 
                                 end_date: datetime, generated_by: int) -> ComplianceReport:
    ...
    def get_compliance_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_violations = session.query(ComplianceViolation).filter(
                ComplianceViolation.created_at >= datetime.utcnow() - timedelta(days=30)
            ).all()
            ...
            for risk_level in AMLRiskLevel:
                count = session.query(User).filter(User.aml_risk_level == risk_level).count()
                aml_summary[risk_level.value] = count
            
            # Get recent large transactions
            large_transactions_count = session.query(Transaction).filter(
                Transaction.created_at >= datetime.utcnow() - timedelta(days=30),
                Transaction.amount >= self.monitoring_thresholds['large_transaction']
            ).count()
            ...
            return {
                'violations': {
                    ...
                    'investigating': len([v for v in recent_violations if v.status == 'investigating']),
                    'resolved': len([v for v in recent_violations if v.status == 'resolved']),
