...
from src.models.user import User, UserStatus, UserTier, KYCStatus, AMLRiskLevel, db
from src.models.portfolio import Portfolio, Asset, AssetType, PortfolioType, RiskLevel
from src.models.ai_models import AIModel, ModelType, ModelStatus
...
def create_indexes():
    ...
    try:
        with db_manager.engine.connect() as conn:
            # User indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_users_email_status ON users(email, status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_users_kyc_status ON users(kyc_status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_users_last_login ON users(last_login)")
            
            # Portfolio indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_portfolios_owner_active ON portfolios(owner_id, is_active)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_portfolios_type_status ON portfolios(portfolio_type, is_active)")
            
            # Transaction indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_transactions_user_date ON transactions(user_id, created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_transactions_portfolio_date ON transactions(portfolio_id, created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_transactions_status_type ON transactions(status, transaction_type)")
            
            # Asset indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_assets_symbol_active ON assets(symbol, is_active)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_assets_type_tradeable ON assets(asset_type, is_tradeable)")
            
            # Holdings indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_holdings_portfolio_active ON portfolio_holdings(portfolio_id, is_active)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_holdings_asset_active ON portfolio_holdings(asset_id, is_active)")
            
            # AI model indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_ai_models_type_status ON ai_models(model_type, status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_predictions_model_timestamp ON model_predictions(model_id, prediction_timestamp)")
            
            # Fraud detection indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_fraud_user_timestamp ON fraud_detections(user_id, created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_fraud_risk_level ON fraud_detections(risk_level)")
...
def create_admin_user():
    ...
    try:
        ...
        admin_user = session.query(User).filter(User.email == 'admin@blockguardian.com').first()
...
def create_sample_assets():
    ...
    try:
        ...
        for asset_data in sample_assets:
            existing_asset = session.query(Asset).filter(Asset.symbol == asset_data['symbol']).first()
...
def create_sample_ai_models():
    ...
    try:
        ...
        sample_models = [
            {
                ...
                'feature_columns': [
                    ...
                    'device_fingerprint', 'location_risk', 'time_of_day'
                ],
            ...
            },
            {
                ...
                'feature_columns': [
                    'portfolio_value', 'asset_allocation', 'transaction_history',
            ...
            },
            {
                ...
                'feature_columns': [
                    ...
                    'volume', 'technical_indicators', 'market_sentiment'
                ],
        ...
        for model_data in sample_models:
            existing_model = session.query(AIModel).filter(
...
def create_demo_user():
    ...
    try:
        ...
        demo_user = session.query(User).filter(User.email == 'demo@blockguardian.com').first()
