...
from src.models.base import Base, AuditMixin, TimestampMixin
...
class AIModel(Base, AuditMixin, TimestampMixin):
    ...
    def deploy(self):
        ...
        # Add audit entry
        self.add_audit_entry("model_deployed", {
    ...
    def deprecate(self, reason: str = None):
        ...
        # Add audit entry
        self.add_audit_entry("model_deprecated", {
            "model_name": self.name,
            "version": self.version,
            "reason": reason,
...
class FraudDetection(Base, AuditMixin, TimestampMixin):
    ...
    model_prediction_id = Column(Integer, ForeignKey('model_predictions.id'), index=True)
    ...
    investigation_status = Column(String(50), default='pending')  # pending, investigating, resolved
    ...
    def take_action(self, action: str, reason: str = None, user_id: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("fraud_action_taken", {
    ...
    def mark_false_positive(self, reason: str, reported_by: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("marked_false_positive", {
            "detection_id": self.detection_id,
            "reason": reason,
...
class RiskAssessment(Base, AuditMixin, TimestampMixin):
    ...
    def expire(self):
        ...
        # Add audit entry
        self.add_audit_entry("assessment_expired", {
...
class AnomalyDetection(Base, AuditMixin, TimestampMixin):
    ...
    # Anomaly details
    anomaly_type = Column(String(100), nullable=False)  # transaction, behavior, market, system
    ...
    resolution_status = Column(String(50), default='open')  # open, investigating, resolved, false_positive
    ...
    def send_alert(self):
        ...
        # Add audit entry
        self.add_audit_entry("anomaly_alert_sent", {
            "detection_id": self.detection_id,
    ...
    def resolve(self, status: str, notes: str = None, resolved_by: int = None):
        ...
        # Add audit entry
        self.add_audit_entry("anomaly_resolved", {
            "detection_id": self.detection_id,
            "status": status,
...
class ModelTrainingJob(Base, AuditMixin, TimestampMixin):
    ...
    # Job configuration
    training_config = Column(JSON, nullable=False)  # Training parameters and configuration
    ...
    def start(self):
        ...
        # Add audit entry
        self.add_audit_entry("training_job_started", {
            "job_id": self.job_id,
    ...
    def complete(self, model_id: int, metrics: Dict[str, Any]):
        ...
        # Add audit entry
        self.add_audit_entry("training_job_completed", {
            "job_id": self.job_id,
            "model_id": model_id,
    ...
    def fail(self, error_message: str):
        ...
        # Add audit entry
        self.add_audit_entry("training_job_failed", {
            "job_id": self.job_id,
