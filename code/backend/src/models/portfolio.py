...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, Enum, Float, ForeignKey, Numeric, Index
...
class Portfolio(Base, AuditMixin, TimestampMixin):
    ...
    performance_snapshots = relationship("PortfolioSnapshot", back_populates="portfolio", lazy='dynamic')
    ...
    def update_portfolio_metrics(self):
        ...
        # Add audit entry
        self.add_audit_entry("metrics_updated", {
...
class Asset(Base, TimestampMixin):
    ...
    def update_price(self, new_price: Decimal, volume: int = None):
        ...
        if self.current_price:
            ...
            if self.current_price > 0:
                self.day_change_percent = float((self.day_change / self.current_price) * 100)
    ...
    def get_price_history(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        prices = self.price_history.filter(
            AssetPrice.timestamp >= start_date
        ).order_by(AssetPrice.timestamp.desc()).all()
...
class PortfolioHolding(Base, AuditMixin, TimestampMixin):
    ...
    def add_shares(self, quantity: Decimal, price: Decimal):
        ...
        # Add audit entry
        self.add_audit_entry("shares_added", {
    ...
    def remove_shares(self, quantity: Decimal, price: Decimal) -> Decimal:
        ...
        # Add audit entry
        self.add_audit_entry("shares_removed", {
            "quantity": float(quantity),
            "price": float(price),
...
class Transaction(Base, AuditMixin, EncryptedMixin, TimestampMixin):
    ...
    def __repr__(self):
        return f'<Transaction {self.transaction_type.value} {self.amount} {self.currency}>'
    ...
    def execute(self):
        ...
        # Add audit entry
        self.add_audit_entry("transaction_executed", {
    ...
    def complete(self, confirmation_number: str = None):
        ...
        # Add audit entry
        self.add_audit_entry("transaction_completed", {
    ...
    def fail(self, reason: str):
        ...
        # Add audit entry
        self.add_audit_entry("transaction_failed", {
            "reason": reason
        })
    ...
    def cancel(self, reason: str = None):
        ...
        # Add audit entry
        self.add_audit_entry("transaction_cancelled", {
            "reason": reason
        })
...
class AssetPrice(Base):
    ...
    def __repr__(self):
        return f'<AssetPrice {self.asset.symbol} {self.close_price} at {self.timestamp}>'
