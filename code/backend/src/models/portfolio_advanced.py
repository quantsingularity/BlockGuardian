...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, Numeric, ForeignKey, Index, CheckConstraint
...
class Portfolio(BaseModel):
        ...
        for holding in self.holdings:
            if holding.is_active and holding.current_value:
                ...
                if position_percentage > self.max_position_size:
                    violations.append({
                        'type': 'position_size',
                        'asset': holding.asset.symbol if holding.asset else 'Unknown',
                        'current_percentage': float(position_percentage * 100),
                        'limit_percentage': float(self.max_position_size * 100),
                        'severity': 'high' if position_percentage > self.max_position_size * 1.5 else 'medium'
                    })
    ...
    def needs_rebalancing(self) -> bool:
        ...
        for asset_class, target_percentage in self.target_allocation.items():
            current_percentage = float(current_allocation.get(asset_class, Decimal('0.00')))
    ...
    def calculate_performance_metrics(self, period_days: int = 365) -> Dict[str, Any]:
        ...
        return {
            ...
            'annualized_return': float(self.annualized_return) if self.annualized_return else 0.0,
            ...
            'expected_shortfall': float(self.expected_shortfall) if self.expected_shortfall else 0.0
        }
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        if include_sensitive:
            result.update({
                ...
                'performance_metrics': self.calculate_performance_metrics(),
                'asset_allocation': {k: float(v) for k, v in self.get_asset_allocation().items()},
                'risk_violations': self.check_risk_violations(),
...
class Asset(BaseModel):
    ...
    price_history = relationship("AssetPrice", back_populates="asset", cascade="all, delete-orphan")
    ...
    def update_price_data(self, price_data: Dict[str, Any]):
        ...
        if self.previous_close and self.current_price:
            ...
            if self.previous_close > 0:
                self.day_change_percent = (self.day_change / self.previous_close * 100).quantize(Decimal('0.01'))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'day_change_percent': float(self.day_change_percent) if self.day_change_percent else None,
            'volume': self.volume,
...
class PortfolioHolding(BaseModel):
    ...
    # Relationships
    portfolio_id = Column(UUID(as_uuid=True), ForeignKey('enhanced_portfolios.id'), nullable=False, index=True)
    asset_id = Column(UUID(as_uuid=True), ForeignKey('enhanced_assets.id'), nullable=False, index=True)
    ...
    # Position tracking
    first_purchase_date = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    last_transaction_date = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    ...
    def update_valuation(self):
        ...
        if self.asset and self.asset.current_price:
            ...
            self.current_value = (self.quantity * self.current_price).quantize(Decimal('0.01'))
            self.unrealized_pnl = self.current_value - self.cost_basis
            
            if self.cost_basis > 0:
                self.unrealized_pnl_percent = ((self.unrealized_pnl / self.cost_basis) * 100).quantize(Decimal('0.01'))
    
    def add_position(self, quantity: Decimal, price: Decimal, transaction_date: datetime = None):
        ...
        # Update position
        self.average_cost = (new_total_cost / new_total_quantity).quantize(Decimal('0.01'))
    ...
    def reduce_position(self, quantity: Decimal, price: Decimal, transaction_date: datetime = None) -> Decimal:
