...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, Enum, Numeric, ForeignKey, Index
...
class User(BaseModel):
    ...
    # KYC/AML compliance
    kyc_status = Column(String(20), default=KYCStatus.NOT_STARTED.value, nullable=False, index=True)
    ...
    aml_risk_level = Column(String(20), default=AMLRiskLevel.LOW.value, nullable=False, index=True)
    ...
    def can_trade(self) -> bool:
        ...
        return (
            self.is_active() and
            self.kyc_status == KYCStatus.APPROVED.value and
            self.aml_risk_level != AMLRiskLevel.PROHIBITED.value
        )
    ...
    def record_login_attempt(self, success: bool, ip_address: str = None, user_agent: str = None):
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        sensitive_fields = [
            'hashed_password', 'mfa_secret', 'backup_codes', 'security_questions',
            'api_key_hash', 'ssn_last_four', 'metadata'
        ]
        ...
        if include_sensitive:
            additional_fields = [
                ...
                'postal_code', 'date_of_birth', 'nationality', 'occupation',
                'employer', 'annual_income', 'net_worth', 'investment_experience',
                'risk_tolerance', 'investment_objectives', 'time_horizon',
                'liquidity_needs', 'daily_transaction_limit', 'monthly_transaction_limit',
                'annual_transaction_limit', 'options_approved', 'margin_approved',
                'crypto_approved', 'international_approved', 'notes'
            ]
