...
class PerformanceMonitor:
    ...
    def record_database_query(self, query_type: str, duration_ms: float, table: str = None):
...
class AlertManager:
    ...
    def add_alert_rule(self, name: str, metric: str, threshold: float, 
                      comparison: str, duration_minutes: int, severity: str):
    ...
    def check_alerts(self):
        ...
        for rule in self.alert_rules:
            ...
            if threshold_exceeded:
                ...
                self.alert_cooldowns[cooldown_key] = current_time + timedelta(minutes=cooldown_minutes)
    
    def _trigger_alert(self, rule: Dict[str, Any], current_value: float, metric_summary: Dict[str, Any]):
        """Trigger an alert"""
        alert = {
        ...
        audit_logger.log_security_alert(
            alert_type=f"monitoring_alert_{rule['severity']}",
            details=alert
        )
    ...
    def _send_alert_notification(self, alert: Dict[str, Any]):
        ...
        logging.warning(f"ALERT: {alert['rule_name']} - {alert['metric']} = {alert['current_value']} (threshold: {alert['threshold']})")
