...
from src.models.portfolio import Portfolio, Asset, PortfolioHolding, Transaction, AssetType, TransactionType, TransactionStatus
from src.security.auth import jwt_required, permission_required, Permission
from src.security.validation import security_validator, ValidationError
from src.security.rate_limiting import rate_limit, RateLimitScope
...
def update_portfolio(portfolio_id):
    ...
    try:
        ...
        try:
            ...
            allowed_fields = [
                'name', 'description', 'investment_objective', 'risk_level',
                'max_position_size', 'max_sector_allocation', 'stop_loss_threshold',
                'auto_rebalance', 'rebalance_frequency'
            ]
    ...
    try:
        ...
        try:
            ...
            asset = session.query(Asset).filter(
                Asset.symbol == data['asset_symbol'].upper()
            ).first()
...
def get_portfolio_performance(portfolio_id):
    ...
    try:
        ...
        try:
            ...
            snapshots = portfolio.performance_snapshots.filter(
                portfolio.performance_snapshots.c.snapshot_date >= start_date
            ).order_by(portfolio.performance_snapshots.c.snapshot_date.asc()).all()
            
            # Calculate performance metrics
            current_value = float(portfolio.total_value)
            total_return = float(portfolio.realized_pnl + portfolio.unrealized_pnl)
            total_return_percent = (total_return / float(portfolio.invested_amount)) * 100 if portfolio.invested_amount > 0 else 0
