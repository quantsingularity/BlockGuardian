...
class AuditLogger:
    ...
    def init_app(self, app):
        ...
        audit_db_uri = current_config.database.uri.replace('/blockguardian', '/blockguardian_audit')
    ...
    def log_data_access(
        ...
        severity = AuditSeverity.HIGH if action.lower() in ['delete', 'export'] else AuditSeverity.MEDIUM
            ...
            for log in logs:
                ...
                if user_id:
                    if user_id not in report['user_activity']:
                        report['user_activity'][user_id] = {'events': 0, 'last_activity': None}
    ...
    def _is_compliance_violation(self, log: Dict[str, Any]) -> bool:
        ...
        violation_patterns = [
            ...
            log['event_type'] == 'access_denied' and log['severity'] in ['high', 'critical'],
            # Data export without proper authorization
            log['event_type'] == 'data_export' and 'unauthorized' in str(log.get('details', '')).lower(),
            # System errors during financial operations
            log['event_type'] == 'system_error' and 'financial' in str(log.get('resource', '')).lower()
        ]
