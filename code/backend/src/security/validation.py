...
class SecurityValidator:
    ...
    def validate_email(self, email: str) -> str:
        ...
        try:
            ...
            raise ValidationError(f"Invalid email address: {str(e)}", "email", "INVALID_FORMAT")
    ...
    def validate_password(self, password: str, require_strong: bool = True) -> bool:
        ...
        if len(password) < 8:
            raise ValidationError("Password must be at least 8 characters long", "password", "TOO_SHORT")
        
        if len(password) > 128:
            raise ValidationError("Password must be less than 128 characters", "password", "TOO_LONG")
        ...
        if password.lower() in weak_passwords:
            raise ValidationError("Password is too common", "password", "COMMON_PASSWORD")
    ...
    def validate_financial_amount(self, amount: Union[str, float, Decimal], min_amount: float = 0.01, max_amount: float = 1000000) -> Decimal:
        """
        ...
        if decimal_amount < Decimal(str(min_amount)):
            raise ValidationError(f"Amount must be at least {min_amount}", "amount", "TOO_SMALL")
        
        if decimal_amount > Decimal(str(max_amount)):
            raise ValidationError(f"Amount must be less than {max_amount}", "amount", "TOO_LARGE")
        
        # Check for reasonable decimal places (max 8 for crypto, 2 for fiat)
        if decimal_amount.as_tuple().exponent < -8:
            raise ValidationError("Amount has too many decimal places", "amount", "TOO_PRECISE")
        
        return decimal_amount
    
    def validate_blockchain_address(self, address: str, network: str = "ethereum") -> str:
        """
        ...
        """
        if not address:
            raise ValidationError("Blockchain address is required", "address", "REQUIRED")
        ...
        if network.lower() == "ethereum":
            if not self.PATTERNS['blockchain_address'].match(address):
                raise ValidationError("Invalid Ethereum address format", "address", "INVALID_FORMAT")
            
            # Validate checksum if present
            if any(c.isupper() for c in address[2:]):  # Has mixed case (checksum)
                if not self._validate_ethereum_checksum(address):
                    raise ValidationError("Invalid Ethereum address checksum", "address", "INVALID_CHECKSUM")
    ...
    def validate_transaction_hash(self, tx_hash: str) -> str:
        ...
        if not self.PATTERNS['transaction_hash'].match(tx_hash):
            raise ValidationError("Invalid transaction hash format", "tx_hash", "INVALID_FORMAT")
    ...
    def validate_phone_number(self, phone: str) -> str:
        ...
        if not self.PATTERNS['phone'].match(cleaned_phone):
            raise ValidationError("Invalid phone number format", "phone", "INVALID_FORMAT")
        
        return cleaned_phone
    
    def validate_date_range(self, start_date: Union[str, date, datetime], end_date: Union[str, date, datetime]) -> Tuple[date, date]:
        """
        Validate date range
        ...
        if isinstance(start_date, str):
            try:
                ...
                raise ValidationError("Invalid start date format", "start_date", "INVALID_FORMAT")
        
        if isinstance(end_date, str):
            try:
                end_date = datetime.fromisoformat(end_date).date()
            except ValueError:
                raise ValidationError("Invalid end date format", "end_date", "INVALID_FORMAT")
        ...
        if (end_date - start_date).days > 3650:
            raise ValidationError("Date range is too large (max 10 years)", "date_range", "RANGE_TOO_LARGE")
    ...
    def validate_json_input(self, json_data: Union[str, dict], max_size: int = 1024 * 1024) -> dict:
        ...
        if isinstance(json_data, str):
            ...
            try:
                ...
                raise ValidationError(f"Invalid JSON format: {str(e)}", "json", "INVALID_FORMAT")
        ...
        try:
            ...
            if current_config.APP_NAME == "production":
                if ip_obj.is_private or ip_obj.is_reserved or ip_obj.is_loopback:
                    raise ValidationError("Private/reserved IP addresses not allowed", "ip_address", "PRIVATE_IP")
            
            return str(ip_obj)
            
        except ValueError:
            raise ValidationError("Invalid IP address format", "ip_address", "INVALID_FORMAT")
    ...
    def validate_url(self, url: str, allowed_schemes: List[str] = None) -> str:
        ...
        try:
            ...
            if not parsed.scheme:
                raise ValidationError("URL must include scheme (http/https)", "url", "MISSING_SCHEME")
            
            if parsed.scheme not in allowed_schemes:
                raise ValidationError(f"URL scheme must be one of: {', '.join(allowed_schemes)}", "url", "INVALID_SCHEME")
            
            if not parsed.netloc:
                raise ValidationError("URL must include domain", "url", "MISSING_DOMAIN")
    ...
    def validate_file_upload(self, file_data: bytes, filename: str, allowed_extensions: List[str] = None, max_size: int = 5 * 1024 * 1024) -> bool:
        """
        ...
        if len(file_data) > max_size:
            raise ValidationError(f"File too large. Maximum size: {max_size} bytes", "file", "TOO_LARGE")
        
        if allowed_extensions:
            file_ext = filename.lower().split('.')[-1] if '.' in filename else ''
            if file_ext not in allowed_extensions:
                raise ValidationError(f"File type not allowed. Allowed types: {', '.join(allowed_extensions)}", "file", "INVALID_TYPE")
