...
from src.models.user import User, UserStatus, UserRole, db
...
@pytest.fixture
def authenticated_user(app, client, sample_user_data):
...
class TestUserLogin:
    ...
    def test_login_account_locked(self, app, client, sample_user_data):
        ...
        with app.app_context():
            ...
            user = db.session.query(User).filter(User.email == sample_user_data['email']).first()
            user.lock_account()
            db.session.commit()
...
class TestMultiFactorAuthentication:
    ...
    def test_setup_mfa(self, client, authenticated_user):
        """Test MFA setup"""
        headers = {'Authorization': f'Bearer {authenticated_user}'}
        
        response = client.post('/api/auth/setup-mfa',
                             headers=headers)
    ...
    def test_enable_mfa_invalid_token(self, client, authenticated_user):
        ...
        response = client.post('/api/auth/enable-mfa',
    ...
    def test_disable_mfa_invalid_password(self, client, authenticated_user, sample_user_data):
        """Test disabling MFA with invalid password"""
        headers = {'Authorization': f'Bearer {authenticated_user}'}
        
        disable_data = {'password': 'wrongpassword'}
        
        response = client.post('/api/auth/disable-mfa',
...
class TestPasswordManagement:
    ...
    def test_change_password_success(self, client, authenticated_user, sample_user_data):
    ...
    def test_change_password_invalid_current(self, client, authenticated_user):
    ...
    def test_change_password_weak_new(self, client, authenticated_user, sample_user_data):
...
class TestProfileManagement:
    ...
    def test_get_profile(self, client, authenticated_user):
