...
from src.models.transaction import Transaction, SuspiciousActivity
...
class TestComplianceManager(TestCase):
    ...
    def test_perform_kyc_verification_success(self, mock_get_session):
        ...
        with patch.object(self.compliance_manager, '_verify_documents') as mock_verify_docs, \
             patch.object(self.compliance_manager, '_verify_identity') as mock_verify_id, \
             patch.object(self.compliance_manager, '_verify_address') as mock_verify_addr, \
             patch.object(self.compliance_manager, '_screen_sanctions') as mock_screen_sanctions, \
             patch.object(self.compliance_manager, '_screen_pep') as mock_screen_pep, \
            ...
            mock_calc_risk.return_value = 25  # Low risk score
    ...
    def test_perform_kyc_verification_failed_step(self, mock_get_session):
        ...
        with patch.object(self.compliance_manager, '_verify_documents') as mock_verify_docs, \
            ...
            # Configure mocks with one failed step
            mock_verify_docs.return_value = {'success': False, 'verified_documents': [], 'issues': ['Missing documents']}
            mock_verify_id.return_value = {'success': True, 'confidence_score': 95, 'issues': []}
            mock_verify_addr.return_value = {'success': True, 'confidence_score': 90, 'issues': []}
            mock_screen_sanctions.return_value = {'success': True, 'matches': [], 'lists_checked': ['ofac_sdn']}
            mock_screen_pep.return_value = {'success': True, 'is_pep': False, 'pep_category': None, 'details': []}
    ...
    def test_verify_documents_success(self):
    ...
    def test_verify_documents_missing_required(self):
    ...
    def test_verify_single_document_valid(self):
    ...
    def test_verify_single_document_expired(self):
        """Test single document verification with expired document"""
        document = {
            'type': 'government_id',
            'document_number': 'P123456789',
            'expiry_date': (datetime.now() - timedelta(days=30)).isoformat(),  # Expired
            'file_path': '/path/to/document.pdf'
        }
    ...
    def test_verify_single_document_missing_fields(self):
        """Test single document verification with missing fields"""
        document = {
            'type': 'government_id',
            'document_number': 'P123456789'
            # Missing expiry_date and file_path
        }
        
        result = self.compliance_manager._verify_single_document(document)
        
        self.assertFalse(result['valid'])
        self.assertTrue(any('Missing required field' in issue for issue in result['issues']))
    ...
    def test_verify_identity_name_mismatch(self):
        """Test identity verification with name mismatch"""
        documents = [
            {
                'type': 'government_id',
                'name': 'Jane Smith'  # Different from test user
            }
        ]
    ...
    def test_verify_address_success(self):
    ...
    def test_verify_address_missing_document(self):
    ...
    def test_screen_sanctions_clean(self):
    ...
    def test_screen_pep_clean(self):
    ...
    def test_calculate_kyc_risk_score_low_risk(self):
        ...
        verification_result = {
            'verification_steps': [
                ...
                {'step': 'sanctions_screening', 'status': 'completed', 'details': {'success': True}},
                {'step': 'pep_screening', 'status': 'completed'}
            ]
        }
        
        # Low risk user (US, old account)
        self.test_user.country = 'US'
        self.test_user.created_at = datetime.now(timezone.utc) - timedelta(days=365)
        
        risk_score = self.compliance_manager._calculate_kyc_risk_score(self.test_user, verification_result)
        
        self.assertLessEqual(risk_score, 30)  # Should be low risk
    
    def test_calculate_kyc_risk_score_high_risk(self):
        ...
        verification_result = {
            'verification_steps': [
                ...
                {'step': 'identity_verification', 'status': 'completed'},
                {'step': 'address_verification', 'status': 'completed'},
                {'step': 'sanctions_screening', 'status': 'failed', 'details': {'success': False}},
                {'step': 'pep_screening', 'status': 'completed'}
            ]
        }
        
        # High risk user (high risk country, new account)
        self.test_user.country = 'AF'  # High risk country
        self.test_user.created_at = datetime.now(timezone.utc) - timedelta(hours=12)  # New account
        
        risk_score = self.compliance_manager._calculate_kyc_risk_score(self.test_user, verification_result)
        
        self.assertGreaterEqual(risk_score, 70)  # Should be high risk
    
    @patch('src.models.base.db_manager.get_session')
    def test_update_user_kyc_status(self, mock_get_session):
        ...
        self.compliance_manager._update_user_kyc_status(self.test_user, verification_result)
    ...
    @patch('src.models.base.db_manager.get_session')
    def test_monitor_transaction_normal(self, mock_get_session):
    ...
    @patch('src.models.base.db_manager.get_session')
    def test_monitor_transaction_large_amount(self, mock_get_session):
        ...
        self.assertTrue(any(flag['type'] == 'large_transaction' for flag in result['flags']))
        self.assertIn('generate_ctr', result['actions_required'])
    
    @patch('src.models.base.db_manager.get_session')
    def test_analyze_transaction_patterns_rapid_transactions(self, mock_get_session):
        """Test transaction pattern analysis for rapid transactions"""
        mock_get_session.return_value = self.mock_session
        
        # Mock recent transactions
        recent_transactions = []
        for i in range(6):  # 6 transactions in last hour
            txn = Mock(spec=Transaction)
            txn.created_at = datetime.now(timezone.utc) - timedelta(minutes=i*10)
            recent_transactions.append(txn)
        
        self.mock_session.query.return_value.filter.return_value.all.return_value = recent_transactions
        
        result = self.compliance_manager._analyze_transaction_patterns(self.test_transaction)
        
        self.assertTrue(result['suspicious'])
        self.assertTrue(any(flag['type'] == 'rapid_transactions' for flag in result['flags']))
        self.assertGreaterEqual(result['risk_score'], 20)
    
    @patch('src.models.base.db_manager.get_session')
    def test_analyze_transaction_patterns_round_amount(self, mock_get_session):
        ...
        self.assertTrue(any(flag['type'] == 'round_amount' for flag in result['flags']))
        self.assertGreaterEqual(result['risk_score'], 10)
    
    @patch('src.models.base.db_manager.get_session')
    def test_analyze_transaction_patterns_structuring(self, mock_get_session):
        """Test transaction pattern analysis for potential structuring"""
        mock_get_session.return_value = self.mock_session
        
        # Mock structuring transactions (multiple transactions just under $10k)
        ...
        self.mock_session.query.return_value.filter.return_value.all.return_value = structuring_transactions
        
        result = self.compliance_manager._analyze_transaction_patterns(self.test_transaction)
        
        self.assertTrue(result['suspicious'])
        self.assertTrue(any(flag['type'] == 'potential_structuring' for flag in result['flags']))
        self.assertIn('generate_sar', result['actions'])
        self.assertGreaterEqual(result['risk_score'], 40)
    
    def test_assess_user_transaction_risk_kyc_not_approved(self):
        """Test user transaction risk assessment for non-approved KYC"""
        self.test_user.kyc_status = 'pending'
        
        result = self.compliance_manager._assess_user_transaction_risk(self.test_user, self.test_transaction)
        
        self.assertGreaterEqual(result['risk_score'], 25)
        self.assertTrue(any(flag['type'] == 'kyc_not_approved' for flag in result['flags']))
    
    def test_assess_user_transaction_risk_high_aml_risk(self):
        ...
        self.assertGreaterEqual(result['risk_score'], 30)
        self.assertTrue(any(flag['type'] == 'high_risk_user' for flag in result['flags']))
    
    def test_assess_user_transaction_risk_vs_income(self):
        """Test user transaction risk assessment vs declared income"""
        self.test_user.kyc_status = 'approved'
        self.test_user.aml_risk_level = 'low'
        self.test_user.annual_income = Decimal('50000')  # Low income
        self.test_transaction.total_amount = Decimal('10000')  # High transaction vs income
        
        result = self.compliance_manager._assess_user_transaction_risk(self.test_user, self.test_transaction)
        
        self.assertGreaterEqual(result['risk_score'], 20)
        self.assertTrue(any(flag['type'] == 'transaction_vs_income' for flag in result['flags']))
    
    @patch('src.models.base.db_manager.get_session')
    def test_generate_sar(self, mock_get_session):
        """Test SAR generation"""
        mock_get_session.return_value = self.mock_session
        ...
        # Verify SAR was created and added to session
        self.mock_session.add.assert_called_once()
        self.mock_session.commit.assert_called_once()
        
        # Verify SAR object
        sar_call = self.mock_session.add.call_args[0][0]
        self.assertIsInstance(sar_call, Mock)  # Would be SuspiciousActivity in real implementation
    
    @patch('src.models.base.db_manager.get_session')
    def test_generate_compliance_report_kyc_summary(self, mock_get_session):
        ...
        report = self.compliance_manager.generate_compliance_report('kyc_summary', start_date, end_date)
        ...
        self.assertEqual(data['kyc_pending'], 2)  # pending + in_progress
        self.assertEqual(data['kyc_rejected'], 1)
    
    @patch('src.models.base.db_manager.get_session')
    def test_generate_compliance_report_aml_activity(self, mock_get_session):
        ...
        self.mock_session.query.return_value.filter.return_value.all.side_effect = [
            mock_transactions, mock_sars
        ]
        
        start_date = datetime.now(timezone.utc) - timedelta(days=30)
        end_date = datetime.now(timezone.utc)
        
        report = self.compliance_manager.generate_compliance_report('aml_activity', start_date, end_date)
    ...
    def test_generate_verification_id(self):
        ...
        self.assertEqual(len(verification_id), 25)  # KYC- + timestamp + - + 8 chars
    
    def test_generate_monitoring_id(self):
        ...
        self.assertEqual(len(monitoring_id), 25)  # AML- + timestamp + - + 8 chars
...
class TestComplianceDataClasses(TestCase):
    ...
    def test_compliance_rule_creation(self):
        ...
        self.assertTrue(rule.enabled)  # Default value
        self.assertIsInstance(rule.conditions, dict)
        self.assertIsInstance(rule.actions, list)
    
    def test_regulatory_requirement_creation(self):
        ...
        self.assertTrue(requirement.mandatory)  # Default value
