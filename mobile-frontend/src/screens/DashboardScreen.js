...
const dashboardItems = [
  ...
  { name: 'Market Analysis', icon: 'analytics-outline', screen: 'MarketAnalysis' },
  { name: 'Blockchain Explorer', icon: 'search-circle-outline', screen: 'BlockchainExplorer' },
  { name: 'AI Recommendations', icon: 'bulb-outline', screen: 'AIRecommendations' },
  { name: 'Security Check', icon: 'shield-checkmark-outline', screen: 'SecurityCheck' }, // Added Security Check link
    ...
    <StyledScrollView className="flex-1 bg-gradient-to-b from-gray-900 to-gray-800">
      <StyledView className="p-6 pt-12">
        <StyledView className="flex-row justify-between items-center mb-8">
          ...
          >
            <Ionicons name={isConnected ? "log-out-outline" : "wallet-outline"} size={18} color="white" style={{ marginRight: 8 }} />
        ...
        <StyledView className="flex-row flex-wrap justify-between">
          {dashboardItems.map((item) => (
            ...
            >
              <Ionicons name={item.icon} size={40} color="#4f46e5" style={{ marginBottom: 10 }} />
              <StyledText className="text-white text-center font-semibold text-base">{item.name}</StyledText>
            </StyledTouchableOpacity>
          ))}
        </StyledView>
        ...
        <StyledView className="mt-4 p-4 bg-gray-700 rounded-lg border border-gray-600">
            <StyledText className="text-xl font-semibold text-white mb-2">Account Summary</StyledText>
            <StyledText className="text-gray-400">Summary details will appear here...</StyledText>
        </StyledView>
