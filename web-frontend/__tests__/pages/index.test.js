...
jest.mock(
  "../../../../BlockGuardian_Project/web-frontend/components/Layout",
  () => {
...
describe("Index Page (Home)", () => {
  ...
  test("renders feature sections with headings and links", () => {
    ...
    expect(
      screen.getByText(
        /Advanced machine learning models for financial prediction/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getAllByRole("link", { name: /Learn more →/i })[0],
    ).toHaveAttribute("href", "/ai-recommendations");
    ...
    expect(
      screen.getAllByRole("link", { name: /Learn more →/i })[1],
    ).toHaveAttribute("href", "/blockchain-explorer");
    ...
    expect(
      screen.getByText(
        /Live financial data streaming with advanced visualization/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getAllByRole("link", { name: /Learn more →/i })[2],
