...
describe('Login Page', () => {
  ...
  test('simulates successful login and redirects', async () => {
    ...
    expect(screen.getByRole('button', { name: /Sign in/i }).querySelector('svg')).toBeInTheDocument(); // Check for spinner svg
  ...
  test('simulates successful signup and redirects', async () => {
    ...
    // Check for loading indicator
    expect(submitButton).toBeDisabled();
    expect(screen.getByRole('button', { name: /Sign up/i }).querySelector('svg')).toBeInTheDocument(); // Check for spinner svg
